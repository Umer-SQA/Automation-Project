export class CardDashboard {
    ...
    goToSpecificCardPage() {
        cy.get('[class="ant-table-row ant-table-row-level-0 row-border fs-18px regular pointer "]').eq(0).should('be.visible').click()
    }
        ...
        cy.get('table').eq(1).then(table => {
            cy.wrap(table)
                ...
                .then(text => {
                    ...
                    cy.get('[class="rc-virtual-list-holder-inner"]').contains(extractedText).click({force:true})
                    //.wait(1000).type(currency)
