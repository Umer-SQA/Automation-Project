...
describe('CardDashboard TS_003 ',function(){
    ...
    it ('TC_CD_008 -Validate that when the user clicks on any transaction from card transaction history, it naviagtes to the specific card transaction page and verify it has correct data', function(){
        cardDashboard.vlidateMainPageHeading()
        cy.get('.ant-tabs-nav-list > :nth-child(3)').should('be.visible').click()
        //cy.get('td:nth-child(7)').should('be.visible')
        cy.get('td:nth-child(7)').eq(1).should('be.visible').invoke('text').then((text) => {
            const currency = text;
            cy.wrap(currency).as('currency')
            cy.log(currency)
        })
        cy.get('td:nth-child(8)').eq(1).should('be.visible').invoke('text').then((text) => {
            const amount = text;
            cy.wrap(amount).as('amount')
            cy.log(amount)
        })
        cy.get('td:nth-child(8)').eq(1).should('be.visible').click({force:true})
        cy.get('.ant-spin-dot.ant-spin-dot-spin').should('not.exist')
        cy.get('[class="ant-input ant-input-disabled"]').invoke('val').then((currency1) => {
            cy.log(currency1)
            cy.get('@currency').then((currency) => {
            expect(currency).to.equal(currency1);
            })
        })
        cy.get('[class="ant-input ant-input-disabled b-g dark-green"]').should('be.visible').invoke('val').then((amount1) => {
          cy.log(amount1); // Log the second value
          cy.get('@amount').then((amount) => {
            expect(amount).to.equal(amount1); // Compare both values
          });
        });
        //cardDashboard.goToSpecificCardPage()
    })
    it ('TC_CD_009 -Validate that clicking on any card from frequently used navigates to the manage specific card page and verify it has same data', function(){
        cardDashboard.vlidateMainPageHeading()
        ...
        .then(($element) => {
            ...
            const nameOnly = modifiedText.replace(/^Mr\.\s*/, ''); // Remove "Mr. " prefix
            cy.log(nameOnly)
            cy.get("[class='slick-slide slick-active slick-current'] [class='ant-typography fs-18px medium dark-green right-align-text p-r-20']").then(($number) => {
                ...
                cy.get('[class="ant-typography fs-18px medium dark-green"]').eq(0).invoke('text').then(($element1) => {
                    ...
                    cy.get('[class="ant-typography fs-18px medium dark-green"]').eq(1).then(($number1) => {
                        ...
                        expect(nameOnly).to.equal(modifiedText1)
    ...
    it.only('TC_CD_018 -Validate that the user cant fund the card if the amount is greater than the available balance from the company Payment balance', function(){
        cardDashboard.vlidateMainPageHeading()
        cardDashboard.fundTheCard()
        cardDashboard.insufficientFundCard('10.00')
    })
